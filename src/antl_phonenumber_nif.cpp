...
static ERL_NIF_TERM format_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    char ref_country_code[MAXBUFLEN];
    string res;
    ...
    if (!enif_get_string(env, argv[0], number, MAXBUFLEN, ERL_NIF_LATIN1) 
      ...
      || !enif_get_string(env, argv[2], ref_country_code, MAXBUFLEN, ERL_NIF_LATIN1)) {
      return enif_make_badarg(env);
    }
    res = format(number, format_name, ref_country_code);
...
static ERL_NIF_TERM get_type_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if (!enif_get_string(env, argv[0], number, MAXBUFLEN, ERL_NIF_LATIN1) || !enif_get_string(env, argv[1], ref_country_code, MAXBUFLEN, ERL_NIF_LATIN1)) {
      return enif_make_badarg(env);
    }

    res = get_type(number, ref_country_code);
    if(res == "parsing error") {
      return enif_make_tuple2(env, enif_make_atom(env, "error"), enif_make_string(env, res.c_str(), ERL_NIF_LATIN1));
    } else {
      return enif_make_tuple2(env, enif_make_atom(env, "ok"), enif_make_string(env, res.c_str(), ERL_NIF_LATIN1));
...
static ERL_NIF_TERM get_country_prefix_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    char number[MAXBUFLEN];
    char ref_country_code[MAXBUFLEN];
    int res;
    ...
    res = get_country_prefix(number, ref_country_code);
    if(res == 0) {
...
static ERL_NIF_TERM to_country_code_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    int country_prefix;
    string country_code;
    
    if (!enif_get_int(env, argv[0], &country_prefix)) {
      return enif_make_badarg(env);
    }
    country_code = to_country_code(country_prefix);
    return enif_make_string(env, country_code.c_str(), ERL_NIF_LATIN1);
}
...
static ERL_NIF_TERM is_valid_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    bool valid = is_valid(number, ref_country_code);
...
static ERL_NIF_TERM is_possible_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    bool possible = is_possible(number, ref_country_code);
...
{
    char country_code[MAXBUFLEN];
    char type[MAXBUFLEN];
    string example_number;
    
    if (!enif_get_string(env, argv[0], country_code, MAXBUFLEN, ERL_NIF_LATIN1) || !enif_get_string(env, argv[1], type, MAXBUFLEN, ERL_NIF_LATIN1)) {
      return enif_make_badarg(env);
    }
    example_number = get_plus_e164_example(country_code, type);
...
static ErlNifFunc nif_funcs[] = {
    ...
    {"get_country_prefix", 2, get_country_prefix_nif},
    {"to_country_code", 1, to_country_code_nif},
